...
class MySubmission(BaseSubmission):
    ...
    def __init__(self, team_name, player_names):
        ...
        for player_name in self.player_names:
            self.agents[player_name] = BotAgent(name=player_name, team_name=team_name)
...
class BotAgent():
    ...
    def __init__(self, name=None, level=3, team_name=None):
        self.name = name
    ...
    def step_level_3(self, obs):
        ...
        if len(my_clone_balls) >= 5 and my_clone_balls[4]['radius'] > 7:
        ...
        if min_thorns_ball is not None:
            ...
            min_distance, min_spore_ball = self.process_food_balls(spore_balls, my_clone_balls[0])
            if min_spore_ball is not None:
                direction = (min_spore_ball['position'] - my_clone_balls[0]['position']).normalize()
            else:
                min_distance, min_food_ball = self.process_food_balls(food_balls, my_clone_balls[0])
                if min_food_ball is not None:
                    direction = (min_food_ball['position'] - my_clone_balls[0]['position']).normalize()
